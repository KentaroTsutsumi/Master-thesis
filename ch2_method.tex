...
There are some ways to approximate the screened Coulomb interaction $v^{\rm screen}(\bm r, \bm r')$. 
...
%In order to compute the Matsubara summation in (\ref{eq:Kelsep}) numerically, a
%frequency cutoff is inevitably introduced. Thanks to the above separation, the numerical error due 
%to introducing the cutoff can be sufficiently suppressed 
%We describe how to calculate the summation with respect to the odd Matsubara frequency in (\ref{eq:Keldyn})
%in Appendix.
...
are derived from the Sham-Shl\"{u}ter connection
...
\begin{figure}[h] %%%%%%% FIGURE F-self-energy
	\centering
...
%
\begin{figure}[h] %%%%%%% FIGURE G-self-energy
	\centering
...
In the previous explanation, we derived the approximate expression of the self-energy from spin fluctuations.
...
%So far we have derived the two-point expression of the effective interaction $\Lambda^{\rm c - \rm d}$.
...
These two expressions are equivalent under the analytic continuation.
...
%	= \frac{1}{\xi_{k}}\tanh\left( \frac{\beta}{2}\xi_{k} \right),
%	\label{eq:GGsum1}
%\end{equation}
%%
%\begin{equation}
...
%	\left( \frac{1}{ {\rm i}\omega_m+E_k'} - \frac{1}{ {\rm i}\omega_m-E_k'} \right)
